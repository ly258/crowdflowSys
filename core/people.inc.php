...
EOF;
		...
		//查选人数rst
		$sql = <<<EOF
				SELECT people FROM "rt_peopleCount" WHERE time 
				in (SELECT max(time) FROM "rt_peopleCount");
EOF;
		
		$prst = fetchOne($link,$sql);
        $prst = json_decode($prst["people"],true);
		//print_r($prst);
		foreach($prst as $value){
			$prst[$value["id"]]["num"] = $value["num"];
			$prst[$value["id"]]["people"] = $value["people"];
		}
		//构造GeoJson
		...
		foreach($cameras as $value){
			...
			if(array_key_exists($value["id"],$prst)){
				$feature["properties"]["num"] = $prst[$value["id"]]["num"];
				...
				//变换为地理坐标
				foreach($feature["properties"]["people"] as $pkey=>$pvalue){
					$coord = $transForm->trans($pvalue);
					$feature["properties"]["people"][$pkey] = $coord;
				}
			}else{
